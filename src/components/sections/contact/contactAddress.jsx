import SlideUp from '@/lib/animations/slideUp'
import React from 'react'
import { FaEnvelope, FaLocationDot, FaPhone } from 'react-icons/fa6'

const ContactAddress = ({handle}) => {
    return (
        <div className="lg:pb-15 lg:pt-15 pb-10 pt-10">
            <div className="container">
                <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-7.5">
                    <SlideUp delay={2}>
                        <div className="bg-background rounded-md shadow-3xl pt-5 pb-7.5 px-7.5 text-center flex flex-col items-center">
                            <div className="w-16 h-16 rounded-full flex justify-center items-center bg-green">
                                <a onClick={handle}>
                                    <span className="text-cream-foreground text-[28px]"><FaLocationDot /> </span>
                                </a>
                            </div>
                            <h5 className="font-bold text-xl mt-5 pb-2.5">Location</h5>
                            <p>Plot 6, Ogombo Road, Off Abraham Adesanya Roundabout, Ajah, Lagos</p>
                        </div>
                    </SlideUp>

                    <SlideUp delay={3}>
                        <div className="bg-background rounded-md shadow-3xl pt-5 pb-7.5 px-7.5 text-center flex flex-col items-center ">
                            <div className="w-16 h-16 rounded-full flex justify-center items-center bg-green">
                            <a href="mailto:info@royalbreedschools.com"><span className="text-cream-foreground text-[28px]"><FaEnvelope /> </span></a>
                            </div>
                            <h5 className="font-bold text-xl mt-5 pb-2.5">Mail</h5>
                            <p><a href="mailto:info@royalbreedschools.com">info@royalbreedschools.com</a> | <a href="mailto:info@royalbreedschools.com">info@royalbreedschools.com</a></p>
                        </div>
                    </SlideUp>

                    <SlideUp delay={4}>
                        <div className="bg-background rounded-md shadow-3xl pt-5 pb-7.5 px-7.5 text-center flex flex-col items-center">
                            <div className="w-16 h-16 rounded-full flex justify-center items-center bg-green">
                                <a href="tel:+2348132103045"><span className="text-cream-foreground text-[28px]"><FaPhone /> </span></a>
                            </div>
                            <h5 className="font-bold text-xl mt-5 pb-2.5">Contact</h5>
                            <p><a href="tel:+2348132103045">+234(0) 813-210-3045</a><br /><a href="">+234(0) 813-210-3045</a> </p>
                        </div>
                    </SlideUp>

                </div>
            </div>
        </div>
    )
}

export default ContactAddress